...
def scrape_ko():
    # names = ["이승만","윤보선","박정희","최규하",'전두환','노태우','김영삼','김대중','노무현','이명박','박근혜','문재인']
    ...
    total_page = results.select_one('div[class="board-count left"] p strong').get_text()
    ...
    for i in trange(1, total_page+1):
        ...
        ref = [i.find('a').get('href') for i in result.findAll('td', class_='subject')]
        res2.close()
        ...
        for link in ref:
            ...
            dict = {'title':a[0],'date':a[1],'president':a[2],'category':a[4],'text':a[6]}
            address.append(dict)
            res3.close()
...
def scrape_us():
    ...
    for i in trange(total_page):
        ...
        ref = [i.find('a').get('href') for i in result1.findAll('td', class_='views-field views-field-title')]
        res1.close() 
        ...
        for url in ref:
            ...
            date=result2.find('span',class_='date-display-single').get('content')
    ...
    print('-' * 18)
    df = pd.DataFrame({'president':presidents, 'date':dates, 'title':titles, 'text':texts})
...
def scrape_jp():
    ...
    for tr in tqdm(tb[2].findAll("tr")[1:]):
        ...
        text = ' '.join([t.text.encode('latin1', 'ignore').decode('utf-8', 'ignore').strip() for t in soup.findAll('p')[1:]])
        texts.append(text)
        time.sleep(1)

    ex = pd.DataFrame({'date': table[2]['年 月 日'], 'title': table[2]['演　説　名'], 'text': texts})
    ex['date'] = ex['date'].replace({'1970月9月21日':'1970年9月21日', '1972年9年11日':'1972年9月11日', '1973月10月2日':'1973年10月2日', '1977年8年18日':'1977年8月18日','1987月1月15日':'1987年1月15日','1994月9月20日':'1994年9月20日','2002年4年21日':'2002年4月21日','2007年1年1月':'2007年1月1日','2007年8年22日':'2007年8月22日','2008年6年13日':'2008年6月13日','202年6月22日':'2022年6月22日'})
    ...
    for i in trange(48,101):
        ...
        titles.extend([i.text.encode('latin1', 'ignore').decode('utf-8', 'ignore') for i in result1.findAll('a')])
        ...
        for link in ref:
            ...
            result2 = BeautifulSoup(res2.text, 'html.parser')
            text = ' '.join([t.text.encode('latin1', 'ignore').decode('utf-8', 'ignore') for t in result2.findAll('p')[1:]])
            texts.append(text)
    ...
    diet['date'] = pd.to_datetime(diet['date'], format='%Y%m%d').dt.strftime('%Y-%m-%d')
